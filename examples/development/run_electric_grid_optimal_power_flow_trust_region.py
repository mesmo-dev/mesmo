...
def run_optimal_operation_problem_trust_region(
    ...
    problem = ElectricGridOptimalOperationProblem(
        scenario_data,
        price_data,
        der_model_set,
        electric_grid_model
    ...
    while (
        ...
        objective_linear_model = (
            problem.optimization_problem.evaluate_objective(problem.optimization_problem.x_vector)
        )
        objective_linear_model = (
            linear_electric_grid_model_set.evaluate_optimization_objective(
                optimization_results,
...
class OptimalOperationProblem(object):
        ...
        if self.electric_grid_model is not None:
            ...
            if self.linear_electric_grid_model_set is None:
                if linear_electric_grid_model_method is mesmo.electric_grid_models.LinearElectricGridModelLocal:
                    ...
                    self.power_flow_solution_set = (
                        mesmo.electric_grid_models.PowerFlowSolutionSet(
                            self.electric_grid_model,
                            self.presolve_results
                        )
                    )
                    # Get linear electric grid model for all timesteps
                    ...
                    power_flow_solution = mesmo.electric_grid_models.PowerFlowSolutionFixedPoint(self.electric_grid_model)
                    self.linear_electric_grid_model_set = (
                        mesmo.electric_grid_models.LinearElectricGridModelSet(
                            self.electric_grid_model,
                            power_flow_solution,
            ...
            self.linear_electric_grid_model_set.define_optimization_problem(
                self.optimization_problem,
                self.price_data
            )

        # Define DER variables and constraints.
        self.der_model_set.define_optimization_problem(
            self.optimization_problem,
            self.price_data
        )
    ...
    def get_results(self) -> mesmo.problems.Results:
        ...
        if self.electric_grid_model is not None:
            self.results.update(
                self.linear_electric_grid_model_set.get_optimization_results(
                    self.optimization_problem
                )
            )
