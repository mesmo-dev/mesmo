...
class DROAmbiguitySet(object):
    ...
    def __init__(
        ...
        delta_index_selected = mesmo.utils.get_index(
            optimization_problem_stage_2.variables, name='price_uncertainty_vector', price_type='energy',
        ...
        self.epsilon_upper_bound.loc[delta_index_selected] = (
            1 * max(
                dro_data_set.energy_price_deviation.min() ** 2,
                dro_data_set.energy_price_deviation.max() ** 2
            )
        )

        # Obtain up-reserve price parameters.
        delta_index_selected = mesmo.utils.get_index(
            optimization_problem_stage_2.variables, name='price_uncertainty_vector', price_type='up_reserve',
            timestep=der_model_set.timesteps
        )
        ...
        delta_index_selected = mesmo.utils.get_index(
            optimization_problem_stage_2.variables, name='price_uncertainty_vector', price_type='down_reserve',
            timestep=der_model_set.timesteps
        )
        self.gamma.loc[delta_index_selected] = dro_data_set.variance_contingency_price
        self.delta_lower_bound.loc[delta_index_selected] = dro_data_set.contingency_reserve_price_deviation.min()
        ...
        delta_index_selected = mesmo.utils.get_index(
            optimization_problem_stage_2.variables, name='disturbance_uncertainty_vector',
            timestep=der_model_set.timesteps, disturbance=der_model_set.disturbances
        )
