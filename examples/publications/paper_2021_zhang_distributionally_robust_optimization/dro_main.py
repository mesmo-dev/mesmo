...
def main():
    ...
    optimization_problem.define_variable(
        'alpha', uncertainty_index=uncertainty_index, timestep=timesteps
    )
    optimization_problem.define_variable(
        'beta', uncertainty_index=uncertainty_index, timestep=timesteps
    )
    optimization_problem.define_variable(
        'sigma', index=[0]
    )
    optimization_problem.define_variable(
        'k_vector_stage_2', stage_2_index=range(len(stage_2.stage_2_index))
    )
    optimization_problem.define_variable(
        'k_matrix_stage_2_delta', uncertainty_index=uncertainty_index, timestep=timesteps,
        stage_2_index=range(len(stage_2.stage_2_index))
    )
    optimization_problem.define_variable(
        'k_matrix_stage_2_epsilon', uncertainty_index=uncertainty_index, timestep=timesteps,
        stage_2_index=range(len(stage_2.stage_2_index))
    )
    ...
    optimization_problem.define_constraint(
        ...
        '>=',
        ('variable', np.array([1.0 + dro_delta_lower_bound.values]), dict(
            name='mu', part='objective', dual_index='1'
        )),
        ('variable', np.array([1.0 - dro_delta_lower_bound.values]), dict(
            name='nu', part='objective', dual_index='1'
        )),
        ('variable', np.array([1.0 - dro_delta_upper_bound.values]), dict(
            name='mu', part='objective', dual_index='2'
        )),
        ('variable', np.array([1.0 + dro_delta_upper_bound.values]), dict(
            name='nu', part='objective', dual_index='2'
        )),
        ('variable', 0.5 * np.ones((1, len(stage_2.delta_index))), dict(
            name='mu', part='objective', dual_index='3a'
        )),
        ('variable', 0.5 * np.ones((1, len(stage_2.delta_index))), dict(
            name='nu', part='objective', dual_index='3'
        )),
        ('variable', np.array([dro_epsilon_upper_bound.values]), dict(
            name='nu', part='objective', dual_index='4'
        ))
    )
    ...
    optimization_problem.define_constraint(
        ...
        ('variable', 1.0, dict(
            name='mu', part='objective', dual_index='3b'
        ))
    )
    ...
    optimization_problem.define_constraint(
        ...
        ('variable', +1.0, dict(
            name='mu', part='objective', dual_index='4'
        ))
    )
    ...
    optimization_problem.define_constraint(
        ...
        '>=',
        ('variable', sp.block_diag([1.0 + np.array([dro_delta_lower_bound.values])] * constraint_len), dict(
            name='mu', part='stage_2', dual_index='1'
        )),
        ('variable', sp.block_diag([1.0 - np.array([dro_delta_lower_bound.values])] * constraint_len), dict(
            name='nu', part='stage_2', dual_index='1'
        )),
        ('variable', sp.block_diag([1.0 - np.array([dro_delta_upper_bound.values])] * constraint_len), dict(
            name='mu', part='stage_2', dual_index='2'
        )),
        ('variable', sp.block_diag([1.0 + np.array([dro_delta_upper_bound.values])] * constraint_len), dict(
            name='nu', part='stage_2', dual_index='2'
        )),
        ('variable', 0.5 * sp.block_diag([np.ones((1, delta_len))] * constraint_len), dict(
            name='mu', part='stage_2', dual_index='3a'
        )),
        ('variable', 0.5 * sp.block_diag([np.ones((1, delta_len))] * constraint_len), dict(
            name='nu', part='stage_2', dual_index='3'
        )),
        ('variable', 1.0 * sp.block_diag([np.array([dro_epsilon_upper_bound.values])] * constraint_len), dict(
            name='nu', part='stage_2', dual_index='4'
        ))
    )
    ...
    optimization_problem.define_constraint(
        ...
        '==',
        ('variable', -1.0 * sp.eye(delta_len * constraint_len), dict(
            name='mu', part='stage_2', dual_index='1'
        )),
        ('variable', +1.0 * sp.eye(delta_len * constraint_len), dict(
            name='nu', part='stage_2', dual_index='1'
        ...
        ('variable', 1.0 * sp.eye(delta_len * constraint_len), dict(
            name='mu', part='stage_2', dual_index='3b'
        )),
    )
    ...
    optimization_problem.define_constraint(
        ...
        '==',
        ('variable', -0.5 * sp.eye(delta_len * constraint_len), dict(
            name='mu', part='stage_2', dual_index='3a'
        )),
        ('variable', +0.5 * sp.eye(delta_len * constraint_len), dict(
            name='nu', part='stage_2',  dual_index='3'
        ...
        ('variable', +1.0 * sp.eye(delta_len * constraint_len), dict(
            name='mu', part='stage_2',  dual_index='4'
        ))
    )
    ...
    soc_constraints_1 = [
        ...
        for soc_i, mu_i, nu_i
        in tqdm(zip(range(len(nu_index)), mu_index, nu_index), total=len(nu_index))
    ]
    ...
    soc_constraints_2 = [
        ...
        for soc_i, mu_ia, mu_ib, nu_i
        in tqdm(zip(range(len(nu_index)), mu_index_a, mu_index_b, nu_index), total=len(nu_index))
    ]
    ...
    x_vector = (
        ...
        x_vector[
            ...
            .str.replace(']', '').str.split('[', expand=True).iloc[:, 1]
            .astype(int).sort_values().index
        ]
    )
