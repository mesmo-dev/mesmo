...
class Stage1(object):
    ...
    def __init__(
        ...
        self.optimization_problem.define_constraint(
            ('variable', 1.0, dict(name='up_reserve_stage_1')),
            '>=',
            ('constant', 0.0)
        )
        self.optimization_problem.define_constraint(
            ('variable', 1.0, dict(name='down_reserve_stage_1')),
            '>=',
            ('constant', 0.0)
        )
        ...
        self.optimization_problem.define_constraint(
            ...
            ('variable', (
                ...
                * np.array([np.real(linear_electric_grid_model_set.electric_grid_model.der_power_vector_reference)])
                ...
                name='der_active_power_vector', scenario='no_reserve_stage_1', timestep=self.der_model_set.timesteps,
        ...
        self.optimization_problem.define_constraint(
            ...
            ('variable', (
                ...
                * np.array([np.real(linear_electric_grid_model_set.electric_grid_model.der_power_vector_reference)])
                ...
                name='der_active_power_vector', scenario='up_reserve_stage_1', timestep=self.der_model_set.timesteps,
                der=self.der_model_set.ders
            )),
            broadcast='timestep'
        )
        self.optimization_problem.define_constraint(
            ...
            ('variable', (
                ...
                * np.array([np.real(linear_electric_grid_model_set.electric_grid_model.der_power_vector_reference)])
                ...
                name='der_active_power_vector', scenario='down_reserve_stage_1', timestep=self.der_model_set.timesteps,
                der=self.der_model_set.ders
            )),
            broadcast='timestep'
        )
        ...
        price_timeseries_energy = (
            dro_data_set.energy_price[0:len(self.der_model_set.timesteps)].to_numpy()
        )
        price_timeseries_reserve = (
            dro_data_set.contingency_reserve_price[0:len(self.der_model_set.timesteps)].to_numpy()
        )
        ...
        self.optimization_problem.define_objective(
            ...
            ('variable', (
                +1.0 * dro_data_set.dro_base_data['prob_up_reserve_bidded'].values
                * np.array([price_timeseries_reserve]) * timestep_interval_hours
            ), dict(name='up_reserve_stage_1', timestep=self.der_model_set.timesteps)),
            ('variable', (
                +1.0 * dro_data_set.dro_base_data['prob_down_reserve_bidded'].values
                * np.array([price_timeseries_reserve]) * timestep_interval_hours
            ), dict(name='up_reserve_stage_1', timestep=self.der_model_set.timesteps))
        )
...
def main():
    ...
    energy_stage_1 = (
        pd.Series(stage_1.optimization_problem.results['energy_stage_1'].values.ravel(), index=stage_1.der_model_set.timesteps)
    )
    up_reserve_stage_1 = (
        pd.Series(stage_1.optimization_problem.results['up_reserve_stage_1'].values.ravel(), index=stage_1.der_model_set.timesteps)
    )
    down_reserve_stage_1 = (
        pd.Series(stage_1.optimization_problem.results['down_reserve_stage_1'].values.ravel(), index=stage_1.der_model_set.timesteps)
    )
