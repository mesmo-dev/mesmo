    ...
    def __init__(
        ...
        for der_name, der_model in self.der_model_set.flexible_der_models.items():
            if isinstance(der_model, mesmo.der_models.FlexibleBuildingModel):
                ...
                der_model.disturbance_delta_mapping_matrix = (
                    pd.DataFrame(
                        0.0,
                ...
                der_model.disturbance_delta_mapping_matrix.loc[
                    [
                        'irradiation_east', 'irradiation_horizontal', 'irradiation_north',
                        'irradiation_south', 'irradiation_west'
                    ],
                    'irradiation'
                ] = 1.0
                der_model.disturbance_delta_mapping_matrix.loc[
        ...
        self.optimization_problem.define_variable(
            'price_uncertainty_vector', scenario=['delta'], price_type=['energy', 'up_reserve', 'down_reserve'],
            timestep=self.der_model_set.timesteps
        )
        self.optimization_problem.define_variable(
            'disturbance_uncertainty_vector', scenario=['delta'], disturbance=self.delta_disturbances,
            timestep=self.der_model_set.timesteps
        )
        ...
        self.optimization_problem.define_constraint(
            ...
            ('variable', 1.0, dict(
                name='energy_deviation_up_stage_2', scenario=['no_reserve_stage_2'],
                timestep=self.der_model_set.timesteps
            )),
            ('variable', -1.0, dict(
                name='energy_deviation_down_stage_2', scenario=['no_reserve_stage_2'],
                timestep=self.der_model_set.timesteps
            )),
            '==',
            ('variable', (
                ...
                * np.array([np.real(linear_electric_grid_model_set.electric_grid_model.der_power_vector_reference)])
                ...
                name='der_active_power_vector', scenario=['no_reserve_stage_2'], timestep=self.der_model_set.timesteps,
        ...
        self.optimization_problem.define_constraint(
            ('variable', 1.0, dict(name='energy_stage_1', timestep=self.der_model_set.timesteps)),
            ('variable', 1.0, dict(
                name='energy_deviation_up_stage_2', scenario=['up_reserve_stage_2'],
                timestep=self.der_model_set.timesteps
            )),
            ('variable', -1.0, dict(
                name='energy_deviation_down_stage_2', scenario=['up_reserve_stage_2'],
                timestep=self.der_model_set.timesteps
            )),
            ...
            ('variable', (
                ...
                * np.array([np.real(linear_electric_grid_model_set.electric_grid_model.der_power_vector_reference)])
                ...
                name='der_active_power_vector', scenario=['up_reserve_stage_2'], timestep=self.der_model_set.timesteps,
                der=linear_electric_grid_model_set.electric_grid_model.ders
            )),
            broadcast='timestep'
        )
        self.optimization_problem.define_constraint(
            ('variable', 1.0, dict(name='energy_stage_1', timestep=self.der_model_set.timesteps)),
            ('variable', 1.0, dict(
                name='energy_deviation_up_stage_2', scenario=['down_reserve_stage_2'],
                timestep=self.der_model_set.timesteps
            )),
            ('variable', -1.0, dict(
                name='energy_deviation_down_stage_2', scenario=['down_reserve_stage_2'],
                timestep=self.der_model_set.timesteps
            )),
            ...
            ('variable', (
                ...
                * np.array([np.real(linear_electric_grid_model_set.electric_grid_model.der_power_vector_reference)])
                ...
                name='der_active_power_vector', scenario=['down_reserve_stage_2'], timestep=self.der_model_set.timesteps,
                der=linear_electric_grid_model_set.electric_grid_model.ders
            )),
            broadcast='timestep'
        )
        ...
        if any(self.der_model_set.states.isin(self.der_model_set.storage_states)):
            self.optimization_problem.define_constraint(
                ('variable', 1.0, dict(
                    name='state_vector', scenario=self.scenarios_stage_2, timestep=self.der_model_set.timesteps[0],
                    state=self.der_model_set.states[self.der_model_set.states.isin(self.der_model_set.storage_states)]
                )),
                '==',
                ('variable', 1.0, dict(
                    name='state_vector', scenario=self.scenarios_stage_2, timestep=self.der_model_set.timesteps[-1],
                    state=self.der_model_set.states[self.der_model_set.states.isin(self.der_model_set.storage_states)]
                )),
                broadcast='scenario'
            )
        ...
        for scenario in self.scenarios_stage_2:
            ...
            # State equation.
            self.optimization_problem.define_constraint(
                ('variable', 1.0, dict(name='state_vector', scenario=[scenario], timestep=self.der_model_set.timesteps[1:])),
                '==',
                ('variable', 'state_matrix', dict(
                    name='state_vector', scenario=[scenario], timestep=self.der_model_set.timesteps[:-1]
                )),
                ('variable', 'control_matrix', dict(
                    name='control_vector', scenario=[scenario], timestep=self.der_model_set.timesteps[:-1]
                )),
                ('constant', 'disturbance_state_equation', dict(scenario=[scenario])),
                ('variable', 'delta_disturbance_matrix', dict(
                    name='disturbance_uncertainty_vector', scenario=['delta'], timestep=self.der_model_set.timesteps[:-1]
                )),
                broadcast=['timestep']
            )
            ...
            # Output equation.
            self.optimization_problem.define_constraint(
                ('variable', 1.0, dict(name='output_vector', scenario=[scenario], timestep=self.der_model_set.timesteps)),
                '==',
                ('variable', 'state_output_matrix', dict(
                    name='state_vector', scenario=[scenario], timestep=self.der_model_set.timesteps
                )),
                ('variable', 'control_output_matrix', dict(
                    name='control_vector', scenario=[scenario], timestep=self.der_model_set.timesteps
                )),
                ('constant', 'disturbance_output_equation', dict(scenario=[scenario])),
                ('variable', 'delta_disturbance_output_matrix', dict(
        ...
        # Output limits.
        self.optimization_problem.define_constraint(
            ('variable', 1.0, dict(name='output_vector', scenario=self.scenarios_stage_2, timestep=self.der_model_set.timesteps)),
            '>=',
        ...
        if len(self.der_model_set.electric_ders) > 0:
            self.optimization_problem.define_constraint(
                ('variable', 1.0, dict(
                    name='der_active_power_vector', scenario=self.scenarios_stage_2, timestep=self.der_model_set.timesteps,
                ...
                ('variable', 'mapping_active_power_by_output', dict(
                    name='output_vector', scenario=self.scenarios_stage_2, timestep=self.der_model_set.timesteps
                )),
                broadcast=['timestep', 'scenario']
            )
            self.optimization_problem.define_constraint(
                ('variable', 1.0, dict(
                    name='der_reactive_power_vector', scenario=self.scenarios_stage_2, timestep=self.der_model_set.timesteps,
                    der=self.der_model_set.electric_ders
                )),
                '==',
        ...
        if len(self.der_model_set.thermal_ders) > 0:
            self.optimization_problem.define_constraint(
                ('variable', 1.0, dict(
                    name='der_thermal_power_vector', scenario=self.scenarios_stage_2, timestep=self.der_model_set.timesteps,
        ...
        self.optimization_problem.define_objective(
            ('variable', -1.0, dict(
                ...
                name='price_uncertainty_vector', price_type='energy', timestep=self.der_model_set.timesteps
            )),
            ('variable', dro_data_set.dro_base_data.at[0, 'prob_up_reserve_activated'], dict(
                name='up_reserve_stage_1', timestep=self.der_model_set.timesteps
            ), dict(
                name='price_uncertainty_vector', price_type='up_reserve', timestep=self.der_model_set.timesteps
            )),
            ('variable', dro_data_set.dro_base_data.at[0, 'prob_down_reserve_activated'], dict(
                name='down_reserve_stage_1', timestep=self.der_model_set.timesteps
            ), dict(
                name='price_uncertainty_vector', price_type='down_reserve', timestep=self.der_model_set.timesteps
            )),
            ('variable', (
                -1.0 * dro_data_set.dro_base_data.at[0, 'penalty_energy_deviation ($/kWh)'] * timestep_interval_hours
            ), dict(
                name='energy_deviation_up_stage_2', timestep=self.der_model_set.timesteps
            )),
            ('variable', (
                -1.0 * dro_data_set.dro_base_data.at[0, 'penalty_energy_deviation ($/kWh)'] * timestep_interval_hours
            ), dict(
                name='energy_deviation_down_stage_2', timestep=self.der_model_set.timesteps
            )),
            ('variable', (
                -0.01 * timestep_interval_hours
            ), dict(
                name='der_active_power_vector', timestep=self.der_model_set.timesteps
            ))
        )
...
def main():
    ...
    optimization_problem.define_constraint(
        ('variable', stage_1.r_matrix_1_stage_1, dict(
            name='x_vector', index=range(len(stage_1.optimization_problem.variables))
        )),
        '<=',
        ('constant', stage_1.t_vector_1)
    )
    optimization_problem.define_objective(
        ('variable', -1.0 * stage_1.w_vector_1_stage_1, dict(
            name='x_vector', index=range(len(stage_1.optimization_problem.variables))
        ))
    )
    ...
    optimization_problem.define_constraint(
        ('variable', 1.0, dict(name='delta_vector')),
        '==',
        ('constant', 0.0)
    )
    optimization_problem.define_constraint(
