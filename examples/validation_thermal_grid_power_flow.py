...
def main():
    ...
    der_thermal_power_vector = (
        pd.DataFrame(index=power_multipliers, columns=thermal_grid_model.ders, dtype=float)
    )
    node_head_vector_method_1 = (
        pd.DataFrame(index=power_multipliers, columns=thermal_grid_model.nodes, dtype=float)
    )
    node_head_vector_method_2 = (
        pd.DataFrame(index=power_multipliers, columns=thermal_grid_model.nodes, dtype=float)
    )
    branch_flow_vector_method_1 = (
        pd.DataFrame(index=power_multipliers, columns=thermal_grid_model.branches, dtype=float)
    )
    branch_flow_vector_method_2 = (
        pd.DataFrame(index=power_multipliers, columns=thermal_grid_model.branches, dtype=float)
    )
    pump_power_method_1 = (
    ...
    node_head_vector_error = (
        100.0 * (
            (node_head_vector_method_1 - node_head_vector_method_2)
            / node_head_vector_method_2
        ).abs().mean(axis='columns')
    )
    branch_flow_vector_error = (
        100.0 * (
            (branch_flow_vector_method_1 - branch_flow_vector_method_2)
            / branch_flow_vector_method_2
        ).mean(axis='columns')
    )
    pump_power_error = (
        100.0 * (
            (pump_power_method_1 - pump_power_method_2)
            / pump_power_method_2
        )
    )
    ...
    for node_index, node in enumerate(thermal_grid_model.nodes):
        ...
        figure.add_trace(go.Scatter(
            x=power_multipliers,
            y=node_head_vector_method_1.loc[:, node],
            name=method_1_label
        ))
        figure.add_trace(go.Scatter(
            x=power_multipliers,
            y=node_head_vector_method_2.loc[:, node],
            name=method_2_label
        ))
        figure.update_layout(
    ...
    for branch_index, branch in enumerate(thermal_grid_model.branches):
        ...
        figure.add_trace(go.Scatter(
            x=power_multipliers,
            y=branch_flow_vector_method_1.loc[:, branch],
            name=method_1_label
        ))
        figure.add_trace(go.Scatter(
            x=power_multipliers,
            y=branch_flow_vector_method_2.loc[:, branch],
            name=method_2_label
        ))
        figure.update_layout(
    ...
    figure.add_trace(go.Scatter(
        x=power_multipliers,
        y=pump_power_method_1,
        name=method_1_label
    ))
    figure.add_trace(go.Scatter(
        x=power_multipliers,
        y=pump_power_method_2,
        name=method_2_label
    ))
    figure.update_layout(
        title=f"Total pump power [W]",
