...
class TestElectricGridModels(unittest.TestCase):
    ...
    def test_power_flow_solution_fixed_point_vs_opendss(self):
        ...
        node_voltage_vector_fixed_point = abs(
            fledge.electric_grid_models.PowerFlowSolutionFixedPoint(fledge.config.config['tests']['scenario_name']).node_voltage_vector
        )
        node_voltage_vector_opendss = abs(
            fledge.electric_grid_models.PowerFlowSolutionOpenDSS(fledge.config.config['tests']['scenario_name']).node_voltage_vector
        )
        time_duration = time.time() - time_start
        logger.info(
        ...
        # Display results.
        if fledge.config.config['tests']['show_plots']:
